...
class LayoutAttachment(Layout):
    ...
    def add_item(self, attachment: Attachment):
        if self._col >= self.col_amount:
            return False

        # Draw image
        img = attachment.picture.resize(self.item_size)
        ...
        text = f"Размер: {'x'.join([str(int(i)) for i in filter(lambda x: x != 0, attachment.size)])}см"
        text_width = font.getlength(text)
        text_position = ((size[0] - text_width) // 2, 0)
        text_image = self._generate_text(text, size, font, text_position)
        self.image.paste(text_image, position)
